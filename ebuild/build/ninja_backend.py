...
class NinjaBackend:
    ...
    def _write_ninja(self) -> None:
        ...
        lines = [
            ...
            # Ninja writes .ninja_log and .ninja_deps into `builddir`; with no
            # builddir it uses the cwd, which is the project root because
            # `ebuild build` invokes ninja with -f from there. That left build
            # state in the source tree, survived `ebuild clean`, and was shared
            # by every --build-dir of the same project.
